...
var VERSION = true ? "4.1.2" : "untranspiled source";
...
// node_modules/@probe.gl/log/dist/probe.js
var Probe = class {
  ...
  getMemoryUsageMB() {
    var _a, _b;
    const performance = (_a = window_) == null ? void 0 : _a.performance;
    const usedJSHeapSize = (_b = performance == null ? void 0 : performance.memory) == null ? void 0 : _b.usedJSHeapSize;
    if (usedJSHeapSize === void 0 || usedJSHeapSize === null) {
      return null;
    }
...
var ProbeLog = class extends BaseLog {
  constructor({ id } = { id: "" }) {
    ...
    this._startTs = probe.getHighResolutionTimer();
...
// node_modules/@luma.gl/core/dist/types.js
...
var VertexFormatDecoder = class {
  ...
  getVertexFormatInfo(format) {
    ...
    let normalizedFormat = format === "unorm8x4-bgra" ? "unorm8x4" : format;
    let webglOnly;
    ...
    let expectedFormat;
    try {
      expectedFormat = webglOnly ? getWebGLOnlyVertexFormat(format, type, components) : this.makeVertexFormat(decodedType.signedType, components, decodedType.normalized);
    } catch {
      throw new Error(`Unsupported vertex format: ${format}`);
    }
...
function getVertexFormatDataTypeInfo(format, type) {
  try {
...
function getVertexFormatComponents(format, componentString) {
  if (!componentString) {
    return 1;
  }
...
function getWebGLOnlyVertexFormat(format, type, components) {
  if (components !== 3) {
    throw new Error(`Unsupported vertex format: ${format}`);
  }
  switch (type) {
...
var WEBGPU_BLEND = 8;
var WEBGPU_STORE = 16;
var WEBGPU_TIER_1_SHIFT = 5;
var WEBGPU_CORE_SHIFT = 10;
var WEBGPU_CREATE_RENDER = WEBGPU_CREATE | WEBGPU_RENDER;
var WEBGPU_CREATE_FILTER = WEBGPU_CREATE | WEBGPU_FILTER;
var WEBGPU_CREATE_RENDER_FILTER_BLEND = WEBGPU_CREATE | WEBGPU_RENDER | WEBGPU_FILTER | WEBGPU_BLEND;
var WEBGPU_CREATE_RENDER_STORE = WEBGPU_CREATE | WEBGPU_RENDER | WEBGPU_STORE;
var WEBGPU_CREATE_FILTER_STORE = WEBGPU_CREATE | WEBGPU_FILTER | WEBGPU_STORE;
var WEBGPU_CREATE_RENDER_FILTER_BLEND_STORE = WEBGPU_CREATE_RENDER_FILTER_BLEND | WEBGPU_STORE;
var WEBGPU_TIER_1_RENDER_BLEND_STORE = (WEBGPU_RENDER | WEBGPU_BLEND | WEBGPU_STORE) << WEBGPU_TIER_1_SHIFT;
var WEBGPU_TIER_1_RENDER_BLEND = (WEBGPU_RENDER | WEBGPU_BLEND) << WEBGPU_TIER_1_SHIFT;
var WEBGPU_TIER_1_STORE = WEBGPU_STORE << WEBGPU_TIER_1_SHIFT;
var WEBGPU_TIER_1_ALL = WEBGPU_CREATE_RENDER_FILTER_BLEND_STORE << WEBGPU_TIER_1_SHIFT;
var WEBGPU_CORE_CREATE_RENDER_FILTER_BLEND = WEBGPU_CREATE_RENDER_FILTER_BLEND << WEBGPU_CORE_SHIFT;
var WEBGPU_CORE_STORE = WEBGPU_STORE << WEBGPU_CORE_SHIFT;
...
var TextureFormatDecoder = class {
  ...
  getWebGPUCapabilities(format) {
    const definition = getTextureFormatDefinition(format);
    if (definition.webgpu !== void 0) {
      return definition.webgpu;
    }
    return this.isCompressed(format) && !format.endsWith("-webgl") ? WEBGPU_CREATE_FILTER2 : 0;
  }
...
// node_modules/@luma.gl/core/dist/adapter/device-defaults.js
var DEVICE_DEFAULT_PROPS = {
  id: null,
...
var _Device = class _Device {
  constructor(props) {
    ...
    __publicField(this, "wgslLanguageFeatures", /* @__PURE__ */ new Set());
...
or create a device with the 'debug: true' prop.`;
   ...
   */
  writeBufferViaCommandEncoder(_commandEncoder, _destinationBuffer, _data, _byteOffset = 0) {
...
__publicField(_Device, "defaultProps", { ...DEVICE_DEFAULT_PROPS });
var Device = _Device;
...
var _Luma = class _Luma {
  constructor() {
    ...
    __publicField(
      ...
      true ? "9.4.0" : "running from source"
    );
...
var _Texture = class _Texture extends Resource {
  ...
  _normalizeCopyElementImageOptions(options_) {
    const optionsWithoutUndefined = _Texture._omitUndefined(options_);
    const mipLevel = optionsWithoutUndefined.mipLevel ?? 0;
    const mipLevelSize = this._getMipLevelSize(mipLevel);
    const options = {
      ..._Texture.defaultCopyElementImageOptions,
      ...mipLevelSize,
      ...optionsWithoutUndefined
    };
    options.width = Math.min(options.width, mipLevelSize.width - options.x);
...
/** Default options */
__publicField(_Texture, "defaultCopyElementImageOptions", {
  element: void 0,
  width: void 0,
  height: void 0,
  sourceX: 0,
...
var _ExternalTexture = class _ExternalTexture extends Resource {
  ...
  constructor(device, props) {
    ...
    this.width = this.props.width || (sourceSize == null ? void 0 : sourceSize.width) || 0;
    this.height = this.props.height || (sourceSize == null ? void 0 : sourceSize.height) || 0;
    this.updateTimestamp = device.incrementTimestamp();
  }
...
var _PipelineFactory = class _PipelineFactory {
  ...
  _getWebGPUAttachmentHash(props) {
    ...
    const depthStencilAttachmentFormat = props.depthStencilAttachmentFormat ?? (((_a = props.parameters) == null ? void 0 : _a.depthWriteEnabled) ? this.device.preferredDepthFormat : null);
...
var _RenderPass = class _RenderPass extends Resource {
  ...
  constructor(device, props, defaultProps = _RenderPass.defaultProps) {
...
// node_modules/@luma.gl/core/dist/adapter/resources/render-bundle.js
var _RenderBundle = class _RenderBundle extends Resource {
  get [Symbol.toStringTag]() {
    return "RenderBundle";
  }
  constructor(device, props) {
    super(device, props, _RenderBundle.defaultProps);
  }
};
__publicField(_RenderBundle, "defaultProps", {
  ...Resource.defaultProps
});
var RenderBundle = _RenderBundle;
var _RenderBundleEncoder = class _RenderBundleEncoder extends RenderPass {
  get [Symbol.toStringTag]() {
  ...
  constructor(device, props = {}) {
    ...
    super(device, normalizeRenderBundleEncoderProps(device, props), _RenderBundleEncoder.bundleDefaultProps);
  }
  /**
...
__publicField(_RenderBundleEncoder, "bundleDefaultProps", {
...
var RenderBundleEncoder = _RenderBundleEncoder;
function validateRenderBundleEncoderProps(props) {
  if (props.sampleCount !== void 0 && props.sampleCount !== 1) {
  ...
  const invalidPropNames = [
    renderPassProps.framebuffer !== void 0 && renderPassProps.framebuffer !== null ? "framebuffer" : null,
    renderPassProps.parameters !== void 0 ? "parameters" : null,
    renderPassProps.clearColor !== void 0 ? "clearColor" : null,
    renderPassProps.clearColors !== void 0 ? "clearColors" : null,
    renderPassProps.clearDepth !== void 0 ? "clearDepth" : null,
    renderPassProps.clearStencil !== void 0 ? "clearStencil" : null,
    renderPassProps.discard !== void 0 ? "discard" : null,
    renderPassProps.occlusionQuerySet !== void 0 ? "occlusionQuerySet" : null,
    renderPassProps.timestampQuerySet !== void 0 ? "timestampQuerySet" : null,
    renderPassProps.beginTimestampIndex !== void 0 ? "beginTimestampIndex" : null,
    renderPassProps.endTimestampIndex !== void 0 ? "endTimestampIndex" : null
  ].filter(Boolean);
...
function normalizeRenderBundleEncoderProps(device, props) {
  return {
    ...
    depthStencilAttachmentFormat: props.depthStencilAttachmentFormat === void 0 ? device.preferredDepthFormat : props.depthStencilAttachmentFormat
  };
}
...
var UniformStore = class {
   ...
   */
  setUniforms(uniforms, commandEncoder) {
    var _a;
   ...
   */
  updateUniformBuffers(commandEncoder) {
    let reason = false;
   ...
   */
  updateUniformBuffer(uniformBufferName, commandEncoder) {
    var _a;
    ...
    if (uniformBuffer && (uniformBlock == null ? void 0 : uniformBlock.needsRedraw)) {
      ...
      if (uniformBuffer) {
        if (commandEncoder) {
          this.device.writeBufferViaCommandEncoder(commandEncoder, uniformBuffer, uniformBufferData);
        } else {
          uniformBuffer.write(uniformBufferData);
        }
      }
...
// node_modules/@luma.gl/core/dist/adapter-utils/buffer-layout-utils.js
function getBufferLayoutAttributeNames(bufferLayout) {
  return bufferLayout.attributes ? bufferLayout.attributes.map((layout) => layout.attribute) : [bufferLayout.name];
}
function getShaderAttributeLocationMap(shaderLayout) {
  return Object.fromEntries(shaderLayout.attributes.map((attribute) => [attribute.name, attribute.location]));
}
function getMinimumAttributeLocation(locations) {
...
function resolveLogicalAttributeMappings(shaderLayout, bufferLayout, options) {
  validateBufferLayouts(bufferLayout);
  const bufferMappingsByAttribute = /* @__PURE__ */ new Map();
  ...
  return shaderLayout.attributes.map((attribute) => {
    const bufferMapping = bufferMappingsByAttribute.get(attribute.name);
    if (!bufferMapping && (options == null ? void 0 : options.warnOnMissingBufferLayout)) {
    ...
    const vertexFormat = (bufferMapping == null ? void 0 : bufferMapping.vertexFormat) || vertexFormatDecoder.getCompatibleVertexFormat(attributeTypeInfo);
    return {
      attributeName: attribute.name,
      bufferName: (bufferMapping == null ? void 0 : bufferMapping.bufferName) || attribute.name,
      location: attribute.location,
      vertexFormat,
      byteOffset: (bufferMapping == null ? void 0 : bufferMapping.byteOffset) ?? 0,
      byteStride: (bufferMapping == null ? void 0 : bufferMapping.byteStride) ?? vertexFormatDecoder.getVertexFormatInfo(vertexFormat).byteLength,
      stepMode: (bufferMapping == null ? void 0 : bufferMapping.stepMode) || attribute.stepMode || (attribute.name.startsWith("instance") ? "instance" : "vertex")
    };
  }).sort((mappingA, mappingB) => mappingA.location - mappingB.location);
}
function validateBufferLayouts(bufferLayouts) {
  for (const bufferLayout of bufferLayouts) {
    if (bufferLayout.attributes && bufferLayout.format || !bufferLayout.attributes && !bufferLayout.format) {
      log.warn(`BufferLayout ${bufferLayout.name} must have either 'attributes' or 'format' field`)();
    }
  }
}
function getBufferLayoutByteStride(bufferLayout) {
  ...
  if (bufferLayout.attributes) {
    ...
    for (const attributeMapping of bufferLayout.attributes) {
      packedByteStride += vertexFormatDecoder.getVertexFormatInfo(attributeMapping.format).byteLength;
    }
...
function getAttributeInfoFromLogicalMapping(shaderLayout, logicalMapping) {
  const shaderDeclaration = getAttributeFromShaderLayout(shaderLayout, logicalMapping.attributeName);
...
//# sourceMappingURL=chunk-LAUF7BHT.js.map
