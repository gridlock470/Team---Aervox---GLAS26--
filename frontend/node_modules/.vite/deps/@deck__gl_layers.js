...
} from "./chunk-VWO4NAEY.js";
import "./chunk-GLLBCJGY.js";
import "./chunk-IWX2KZTC.js";
import {
  CubeGeometry,
  Geometry,
  Model,
...
// node_modules/@deck.gl/layers/dist/arc-layer/arc-layer.wgsl.js
var shaderWGSL = (
  /* wgsl */
...
// node_modules/@deck.gl/layers/dist/bitmap-layer/bitmap-layer.wgsl.js
...
var uniformBlock2 = `layout(std140) uniform iconUniforms {
  float sizeScale;
...
function getShaderWGSL(useRowIndexes) {
  return shaderWGSL2.replace("PICKING_COLOR_ATTRIBUTE", useRowIndexes ? "@location(10) rowIndexes: u32," : "").replace("PICKING_COLOR_VALUE", useRowIndexes ? "picking_getPickingColorFromIndex(inp.rowIndexes)" : "picking_getPickingColorFromIndex(inp.instanceIndex)");
}
...
var LineLayer = class extends layer_default {
  ...
  getShaders() {
    ...
    return super.getShaders({
      ...
      source: shaderWGSL3,
      defines: antialiasing ? { ANTIALIASING: 1 } : {},
      modules: [project32_default, color_default, picking_default, lineUniforms]
    });
  }
  // This layer has its own wrapLongitude logic
  get wrapLongitude() {
...
function getShaderWGSL2(useRowIndexes) {
  return shaderWGSL5.replace("PICKING_COLOR_ATTRIBUTE", useRowIndexes ? "@location(8) rowIndexes: u32," : "").replace("PICKING_COLOR_VALUE", useRowIndexes ? "picking_getPickingColorFromIndex(attributes.rowIndexes)" : "picking_getPickingColorFromIndex(attributes.instanceIndex)");
}

// node_modules/@deck.gl/layers/dist/utils/clip-extension.js
  ...
  getShaders() {
    ...
    return super.getShaders({
      ...
      modules: [
        ...
        picking_default,
        scatterplotUniforms,
        ...this.context.device.type === "webgpu" ? [clip_extension_default] : []
      ]
    });
  }
  initializeState() {
...
function push(target, source2) {
  const size = source2.length;
  const startIndex = target.length;
  if (startIndex > 0) {
    let isDuplicate = true;
    for (let i = 0; i < size; i++) {
      if (target[startIndex - size + i] !== source2[i]) {
        isDuplicate = false;
        break;
      }
    }
  ...
  for (let i = 0; i < size; i++) {
    target[startIndex + i] = source2[i];
  }
  return true;
}
function copy(target, source2) {
  const size = source2.length;
  for (let i = 0; i < size; i++) {
    target[i] = source2[i];
  }
...
var uniformBlock3 = `layout(std140) uniform columnUniforms {
  float radius;
  float angle;
  vec2 offset;
  bool extruded;
...
// node_modules/@deck.gl/layers/dist/column-layer/column-layer.wgsl.js
var sharedSource = (
  /* wgsl */
  `struct Attributes {
  @builtin(instance_index) instanceIndex: u32,
...
var PathTesselator = class extends Tesselator {
  ...
  getPathSegmentIndices(dataIndex) {
    const segmentTypes = this.attributes.segmentTypes;
    const rowStart = this.vertexStarts[dataIndex];
    const rowEnd = Math.min(this.vertexStarts[dataIndex + 1] ?? this.instanceCount, this.instanceCount);
    const result = [];
...
var uniformBlockGLSL3 = `layout(std140) uniform pathUniforms {
  float widthScale;
  float widthMinPixels;
  float widthMaxPixels;
  float jointType;
...
// node_modules/@deck.gl/layers/dist/path-layer/path-layer.wgsl.js
var shaderWGSL6 = (
  /* wgsl */
...
function getPathProjectionScale(viewport) {
...
function pathProjectionScalesEqual(left, right) {
...
var PathLayer = class extends layer_default {
  ...
  getPathProjectionScale(viewport) {
    var _a;
    const coordinateSystem = this.props.coordinateSystem;
    const hasDashMetrics = Boolean((_a = this.getAttributeManager()) == null ? void 0 : _a.getAttributes().instanceDashOffsets);
    if (!hasDashMetrics) {
      return null;
    }
    const trackViewportScale = viewport instanceof web_mercator_viewport_default && viewport.zoom >= 12 && // Offset coordinate systems use their fixed coordinateOrigin as the projection origin.
    // Default/lnglat and preprojected Cartesian paths follow the viewport center instead.
    (coordinateSystem === "default" || coordinateSystem === "lnglat" || coordinateSystem === "cartesian");
    if (trackViewportScale) {
      const projectUniforms = project_default.getUniforms({
        viewport,
        coordinateSystem,
    ...
    return projectionScale ? [viewport.projectionMode, ...projectionScale] : [viewport.projectionMode];
  }
  shouldUpdateState(params) {
    var _a, _b;
    const { viewport } = this.context;
    return super.shouldUpdateState(params) || ((_a = this.state) == null ? void 0 : _a.tessellationResolution) !== viewport.resolution || !pathProjectionScalesEqual((_b = this.state) == null ? void 0 : _b.pathProjectionScale, this.getPathProjectionScale(viewport));
  }
  initializeState() {
    const noAlloc = true;
  ...
  updateState(params) {
    ...
    const pathProjectionScaleChanged = !pathProjectionScalesEqual(this.state.pathProjectionScale, pathProjectionScale);
    const getPathChanged = changeFlags.updateTriggersChanged && (changeFlags.updateTriggersChanged.all || changeFlags.updateTriggersChanged.getPath);
    const geometryConfigurationChanged = getPathChanged || props._pathType !== oldProps._pathType || props.positionFormat !== oldProps.positionFormat || props.wrapLongitude !== oldProps.wrapLongitude || tessellationResolutionChanged;
    ...
    if (geometryChanged) {
      ...
      pathTesselator.updateGeometry({
        ...
        dataChanged: geometryConfigurationChanged ? void 0 : changeFlags.dataChanged
      });
      this.setState({
...
var uniformBlock4 = `layout(std140) uniform solidPolygonUniforms {
  bool extruded;
  bool isWireframe;
  float elevationScale;
} solidPolygon;
...
// node_modules/@deck.gl/layers/dist/solid-polygon-layer/solid-polygon-layer.wgsl.js
...
var SolidPolygonLayer = class extends layer_default {
  ...
  _getModels() {
    ...
    if (filled) {
      ...
      if (this.context.device.type === "webgpu") {
        bufferLayout = bufferLayout.filter((layout) => layout.name !== "indices" && layout.name !== "vertexValid" && layout.name !== "instanceVertexValid" && layout.name !== "nextVertexPositions");
      }
...
var uniformBlock5 = `layout(std140) uniform sdfUniforms {
  float gamma;
  bool enabled;
  float buffer;
  float outlineBuffer;
...
// node_modules/@deck.gl/layers/dist/text-layer/multi-icon-layer/multi-icon-layer.wgsl.js
...
var shaderWGSL7 = getShaderWGSL3();
...
var MultiIconLayer = class extends icon_layer_default {
  ...
  updateState(params) {
    ...
    if (changeFlags.extensionsChanged) {
      (_a = this.state.fillModel) == null ? void 0 : _a.destroy();
      const fillModel = this.context.device.type === "webgpu" ? this._getModel(`${this.props.id}-fill`) : void 0;
      this.setState({
        fillModel,
  ...
  draw(params) {
    ...
    if (sdf && outlineWidth) {
      ...
      if (iconsTexture) {
        ...
        fillModel.shaderInputs.setProps({
          sdf: { ...sdfProps, outlineBuffer: DEFAULT_BUFFER2 },
          text: textProps
        });
...
var uniformBlock6 = `layout(std140) uniform textBackgroundUniforms {
  bool billboard;
  float sizeScale;
  float sizeMinPixels;
  float sizeMaxPixels;
...
// node_modules/@deck.gl/layers/dist/text-layer/text-background-layer/text-background-layer.wgsl.js
