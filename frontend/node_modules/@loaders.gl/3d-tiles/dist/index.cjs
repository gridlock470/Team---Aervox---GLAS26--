  ...
  _getTypedArrayFromArray(propertyName, componentType, array) {
    ...
    if (!typedArray) {
      if (ArrayBuffer.isView(array)) {
        ...
        const elementCount = byteLength / import_math.GLType.getByteSize(componentType);
        typedArray = import_math.GLType.createTypedArray(componentType, array.buffer, byteOffset, elementCount);
      } else if (array instanceof ArrayBuffer) {
        typedArray = import_math.GLType.createTypedArray(componentType, array);
      } else {
        const ArrayType = import_math.GLType.getArrayType(componentType);
...
async function extractGLTF(tile, gltfFormat, options, context) {
  ...
  if (tile3DOptions.loadGLTF) {
    ...
    if (tile.gltfUrl) {
      const { fetch } = context;
      const response = await fetch(tile.gltfUrl, options == null ? void 0 : options.core);
...
var Tiles3DArchive = class extends import_zip.IndexedArchive {
  ...
  async getFileBytes(path3) {
    ...
    if (this.hashTable) {
      ...
      const localFileHeader = await (0, import_zip.parseZipLocalFileHeader)(byteOffset, this.file);
      if (!localFileHeader) {
        return null;
      }
      const compressedFile = await (0, import_zip.readRange)(this.file, localFileHeader.fileDataOffset, localFileHeader.fileDataOffset + localFileHeader.compressedSize);
...
async function parse3DTilesArchive2(data, options = {}) {
  var _a;
  const archive = await parse3DTilesArchive(new import_zip3.DataViewReadableFile(new DataView(data)));
...
function encode3DTileHeader(tile, dataView, byteOffset) {
  ...
  (0, import_loader_utils10.assert)(Array.isArray(magic) && Number.isFinite(version) && Number.isFinite(byteLength));
...
function encodeBatchedModel3DTile(tile, dataView, byteOffset, options) {
  ...
  const featureTableJsonByteLength = (0, import_loader_utils11.padToNBytes)(featureTableJsonString.length, 8);
  const batchTableJsonByteLength = batchTableJsonString ? (0, import_loader_utils11.padToNBytes)(batchTableJsonString.length, 8) : 0;
  ...
  byteOffset += 16;
  byteOffset = (0, import_loader_utils11.copyPaddedStringToDataView)(dataView, byteOffset, featureTableJsonString, 8);
  if (batchTable) {
    byteOffset = (0, import_loader_utils11.copyPaddedStringToDataView)(dataView, byteOffset, batchTableJsonString, 8);
  }
  const gltfEncoded = tile.gltfEncoded;
  if (gltfEncoded) {
    byteOffset = (0, import_loader_utils11.copyBinaryToDataView)(dataView, byteOffset, gltfEncoded, gltfEncoded.byteLength);
  }
  encode3DTileByteLength(dataView, byteOffsetStart, byteOffset - byteOffsetStart);
  return byteOffset;
}
...
function encodeInstancedModel3DTile(tile, dataView, byteOffset, options) {
  ...
  byteOffset += 20;
  byteOffset += (0, import_loader_utils12.copyStringToDataView)(dataView, byteOffset, featureTableJsonString, featureTableJsonByteLength);
  byteOffset += (0, import_loader_utils12.copyStringToDataView)(dataView, byteOffset, gltfUri, gltfUriByteLength);
  encode3DTileByteLength(dataView, byteOffsetStart, byteOffset - byteOffsetStart);
  return byteOffset;
}
...
function encodePointCloud3DTile(tile, dataView, byteOffset, options) {
  ...
  featureTableJsonString = (0, import_loader_utils13.padStringToByteAlignment)(featureTableJsonString, 4);
  ...
  byteOffset += (0, import_loader_utils13.copyBinaryToDataView)(dataView, byteOffset, featureTableBinary, featureTableBinaryByteLength);
  encode3DTileByteLength(dataView, byteOffsetStart, byteOffset - byteOffsetStart);
  return byteOffset;
}
...
function encode3DTileToDataView(tile, dataView, byteOffset, options) {
  (0, import_loader_utils14.assert)(typeof tile.type === "string");
  switch (tile.type) {
    case TILE3D_TYPE.COMPOSITE:
