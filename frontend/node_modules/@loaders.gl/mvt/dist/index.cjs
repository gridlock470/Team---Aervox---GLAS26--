...
__export(dist_exports, {
  ...
  MapStyleSchema: () => MapStyleSchema,
  ResolvedMapStyleSchema: () => ResolvedMapStyleSchema,
  TableTileSource: () => TableTileSource,
  TileJSONLoader: () => TileJSONLoader,
  resolveMapStyle: () => resolveMapStyle
});
...
var MapStyleSourceSchemaInternal = import_zod.z.object({
...
var MapStyleLayerSchemaInternal = import_zod.z.object({
...
var MapStyleSchema = import_zod.z.object({
  version: import_zod.z.number().optional(),
  metadata: import_zod.z.record(import_zod.z.string(), import_zod.z.unknown()).optional(),
  sources: import_zod.z.record(import_zod.z.string(), MapStyleSourceSchemaInternal).optional(),
  layers: import_zod.z.array(MapStyleLayerSchemaInternal).optional()
}).catchall(import_zod.z.unknown());
var ResolvedMapStyleSchema = import_zod.z.object({
  version: import_zod.z.number().optional(),
  metadata: import_zod.z.record(import_zod.z.string(), import_zod.z.unknown()).optional(),
  sources: import_zod.z.record(import_zod.z.string(), MapStyleSourceSchemaInternal),
  layers: import_zod.z.array(MapStyleLayerSchemaInternal)
}).catchall(import_zod.z.unknown());
var MapStyleSourceSchema = MapStyleSourceSchemaInternal;
...
function normalizeMapStyleUrl(url, baseUrl) {
  if (!url) {
    return url;
  }
  ...
  if (isAbsoluteFilePath(baseUrl)) {
    const basePath = baseUrl.endsWith("/") ? baseUrl : import_loader_utils.path.dirname(baseUrl);
    return import_loader_utils.path.resolve(basePath, url);
  }
  try {
    return decodeURI(new URL(url, baseUrl).toString());
  } catch {
...
function normalizeMapStyleTiles(tiles, baseUrl) {
  return Array.isArray(tiles) ? tiles.map((tile) => normalizeMapStyleUrl(tile, baseUrl) || tile) : tiles;
}
async function fetchMapStyleJson(url, loadOptions) {
  const fetchFunction = (loadOptions == null ? void 0 : loadOptions.fetch) || fetch;
  const response = await fetchFunction(url, loadOptions == null ? void 0 : loadOptions.fetchOptions);
  if (!response.ok) {
    throw new Error(`Failed to load map style resource: ${url} (${response.status})`);
  }
  return await response.json();
}
async function resolveMapStyleSource(source, baseUrl, loadOptions) {
  if (!source) {
    return source;
  }
...
async function resolveMapStyle(style, options) {
  const mapStyleOptions = getMapStyleLoadOptions(options);
  const styleDefinition = MapStyleSchema.parse(typeof style === "string" ? await fetchMapStyleJson(style, mapStyleOptions) : structuredClone(style));
  ...
  await Promise.all(Object.entries(styleDefinition.sources || {}).map(async ([sourceId, source]) => {
    resolvedSources[sourceId] = await resolveMapStyleSource(source, baseUrl, mapStyleOptions) || {};
  }));
...
var VERSION2 = true ? "4.5.1" : "latest";
var MapStyleLoader = {
  dataType: null,
  batchType: null,
  name: "Map Style",
...
function projectToLngLatFlat(data, tileIndex, extent) {
  const { x, y, z: z2 } = tileIndex;
  const size = extent * Math.pow(2, z2);
  const x0 = extent * x;
  const y0 = extent * y;
...
function checkOptions(options) {
  ...
  if (options.gis) {
    import_loader_utils2.log.warn('MVTLoader: "options.gis" is deprecated, use "options.mvt.shape" instead')();
  }
  return options.mvt;
}
...
// dist/mvt-loader.js
var VERSION3 = true ? "4.5.1" : "latest";
var MVTWorkerLoader = {
  ...MVTFormat,
  dataType: null,
  batchType: null,
...
function fromVectorTileJs(tile) {
  ...
  return (0, import_loader_utils3.copyToArrayBuffer)(uint8Array.buffer, uint8Array.byteOffset, uint8Array.byteOffset + uint8Array.byteLength);
}
...
// dist/mvt-writer.js
var VERSION4 = true ? "4.5.1" : "latest";
var MVTWriter = {
  ...MVTFormat,
  version: VERSION4,
  binary: true,
...
var MVTTileSource = class extends import_loader_utils4.DataSource {
  metadataUrl = null;
  schema = "tms";
  metadata;
  extension;
  ...
  async getTile(parameters) {
    const { x, y, z: z2 } = parameters;
    const tileUrl = this.getTileURL(x, y, z2);
  ...
  async getTileData(parameters) {
    const { x, y, z: z2 } = parameters.index;
    const arrayBuffer = await this.getTile({ x, y, z: z2, layers: [] });
    if (arrayBuffer === null) {
      return null;
    }
    ...
    switch (this.mimeType) {
      case "application/vnd.mapbox-vector-tile":
        return await this._parseVectorTile(arrayBuffer, { x, y, z: z2, layers: [] });
      default:
  ...
  getTileURL(x, y, z2) {
    switch (this.schema) {
      case "xyz":
        return `${this.url}/${x}/${y}/${z2}${this.extension}`;
      case "tms":
        return `${this.url}/${z2}/${x}/${y}${this.extension}`;
      case "template":
        return getURLFromTemplate(this.url, x, y, z2, "0");
      default:
...
function getURLFromTemplate(template, x, y, z2, id = "0") {
  ...
  url = url.replace(zRegex, String(z2));
  if (Number.isInteger(y) && Number.isInteger(z2)) {
    url = url.replace(/\{-y\}/g, String(Math.pow(2, z2) - y - 1));
  }
  return url;
}
...
function createProtoTile(features2, z2, tx, ty, options) {
  const tolerance = z2 === options.maxZoom ? 0 : options.tolerance / ((1 << z2) * options.extent);
  const tile = {
    protoFeatures: [],
    sourceFeatures: null,
    numPoints: 0,
...
var _TableVectorTileSource = class extends import_loader_utils5.DataSource {
  ...
  constructor(table, options) {
    ...
    this.tableOptions = (0, import_loader_utils5.getRequiredOptions)(this.options).table;
    this.getTileData = this.getTileData.bind(this);
  ...
  async getVectorTile(tileIndex) {
    ...
    import_loader_utils5.log.info(2, "getVectorTile", tileIndex, table)();
    return table;
  }
  ...
  async getTileData(tileParams) {
    const { x, y, z: z2 } = tileParams.index;
    const tile = await this.getVectorTile({ x, y, z: z2 });
  ...
  createRootTiles(table) {
    ...
    import_loader_utils5.log.log(1, "TableVectorTileSource creating root tiles", this.tableOptions)();
    import_loader_utils5.log.time(1, "preprocess table")();
    let features2 = convertFeaturesToProtoFeature(table, this.tableOptions);
    import_loader_utils5.log.timeEnd(1, "preprocess table")();
    import_loader_utils5.log.time(1, "generate tiles")();
    features2 = wrapFeatures(features2, this.tableOptions);
    if (features2.length === 0) {
      import_loader_utils5.log.log(1, "TableVectorTileSource: no features generated")();
      return;
    }
    this.splitTile(features2, 0, 0, 0);
    const rootTile = this.tiles[0];
    import_loader_utils5.log.log(1, `root tile features: ${rootTile.numFeatures}, points: ${rootTile.numPoints}`)();
    import_loader_utils5.log.timeEnd(1, "generate tiles")();
    import_loader_utils5.log.log(1, `TableVectorTileSource: tiles generated: ${this.stats.get("total").count}`, this.stats)();
  }
  /**
  ...
  getProtoTile(tileIndex) {
    ...
    const z22 = 1 << z2;
    x = x + z22 & z22 - 1;
    const id = toID(z2, x, y);
    if (this.tiles[id]) {
      return transformTile(this.tiles[id], extent);
    }
    import_loader_utils5.log.log(import_loader_utils5.log, "drilling down to z%d-%d-%d", z2, x, y)();
    let z0 = z2;
    let x0 = x;
    let y0 = y;
    let parent;
    ...
    import_loader_utils5.log.log(1, "found parent tile z%d-%d-%d", z0, x0, y0)();
    import_loader_utils5.log.time(1, "drilling down")();
    this.splitTile(parent.sourceFeatures, z0, x0, y0, z2, x, y);
    import_loader_utils5.log.timeEnd(1, "drilling down")();
  ...
  splitTile(features2, z2, x, y, cz, cx, cy) {
    const stack = [features2, z2, x, y];
    while (stack.length) {
      y = stack.pop();
      x = stack.pop();
      z2 = stack.pop();
      ...
      if (!tile) {
        import_loader_utils5.log.time(2, "tile creation")();
        tile = this.tiles[id] = createProtoTile(features2, z2, x, y, this.tableOptions);
        this.tileCoords.push({ z: z2, x, y });
        const key = `z${z2}`;
        ...
        stat.incrementCount();
        import_loader_utils5.log.log(2, "tile z%d-%d-%d (features: %d, points: %d, simplified: %d)", z2, x, y, tile.numFeatures, tile.numPoints, tile.numSimplified)();
        import_loader_utils5.log.timeEnd(2, "tile creation")();
      }
      tile.sourceFeatures = features2;
      if (cz === void 0) {
        if (z2 === this.tableOptions.indexMaxZoom || tile.numPoints <= this.tableOptions.maxPointsPerTile) {
          continue;
        }
      } else if (z2 === this.tableOptions.maxZoom || z2 === cz) {
        continue;
      } else if (cz !== void 0) {
        const zoomSteps = cz - z2;
      ...
      if (features2.length === 0)
        continue;
      import_loader_utils5.log.time(2, "clipping tile")();
      ...
      let tr = null;
      let br = null;
      let left = clipFeatures(features2, z22, x - k1, x + k3, 0, tile.minX, tile.maxX, this.tableOptions);
      let right = clipFeatures(features2, z22, x + k2, x + k4, 0, tile.minX, tile.maxX, this.tableOptions);
      features2 = null;
      if (left) {
        tl = clipFeatures(left, z22, y - k1, y + k3, 1, tile.minY, tile.maxY, this.tableOptions);
        bl = clipFeatures(left, z22, y + k2, y + k4, 1, tile.minY, tile.maxY, this.tableOptions);
        left = null;
      }
      if (right) {
        tr = clipFeatures(right, z22, y - k1, y + k3, 1, tile.minY, tile.maxY, this.tableOptions);
        br = clipFeatures(right, z22, y + k2, y + k4, 1, tile.minY, tile.maxY, this.tableOptions);
        right = null;
      }
      import_loader_utils5.log.timeEnd(2, "clipping tile")();
      stack.push(tl || [], z2 + 1, x * 2, y * 2);
      stack.push(bl || [], z2 + 1, x * 2, y * 2 + 1);
      stack.push(tr || [], z2 + 1, x * 2 + 1, y * 2);
      stack.push(br || [], z2 + 1, x * 2 + 1, y * 2 + 1);
    }
  }
};
...
function toID(z2, x, y) {
  return ((1 << z2) * y + x) * 32 + z2;
}
