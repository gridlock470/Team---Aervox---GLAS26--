...
var import_decoder = require("meshoptimizer/decoder");
...
async function meshoptDecodeGltfBuffer(target, count, byteStride, source, mode, filter = "NONE") {
  await import_decoder.MeshoptDecoder.ready;
  import_decoder.MeshoptDecoder.decodeGltfBuffer(target, count, byteStride, source, mode, getMeshoptCompressionFilter(filter));
}

// dist/lib/extensions/meshopt-compression.js
async function decodeMeshoptCompression(gltfData, options, extensionName) {
  var _a;
  if (!((_a = options.gltf) == null ? void 0 : _a.decompressMeshes) || !options.gltf.loadBuffers) {
    return;
  }
  validateMeshoptCompressionExclusivity(gltfData.json);
  const scenegraph = new GLTFScenegraph(gltfData);
  const bufferViews = gltfData.json.bufferViews || [];
  const promises = bufferViews.map((bufferView) => decodeMeshoptBufferView(scenegraph, bufferView, extensionName));
...
function validateMeshoptCompressionExclusivity(gltf) {
  ...
  for (let bufferViewIndex = 0; bufferViewIndex < bufferViews.length; bufferViewIndex++) {
    ...
    if ((extensions == null ? void 0 : extensions.KHR_meshopt_compression) && extensions.EXT_meshopt_compression) {
      throw new Error(`glTF bufferView ${bufferViewIndex} cannot use both KHR_meshopt_compression and EXT_meshopt_compression.`);
    }
  }
  ...
  for (let bufferIndex = 0; bufferIndex < buffers.length; bufferIndex++) {
    ...
    if ((extensions == null ? void 0 : extensions.KHR_meshopt_compression) && extensions.EXT_meshopt_compression) {
      throw new Error(`glTF buffer ${bufferIndex} cannot use both KHR_meshopt_compression and EXT_meshopt_compression.`);
    }
  }
}
async function decodeMeshoptBufferView(scenegraph, bufferView, extensionName) {
  ...
  const { byteOffset = 0, byteLength, byteStride, count, mode, filter = "NONE", buffer: sourceBufferIndex } = meshoptExtension;
  const sourceBuffer = scenegraph.gltf.buffers[sourceBufferIndex];
  const targetBuffer = scenegraph.gltf.buffers[bufferView.buffer];
  const source = new Uint8Array(sourceBuffer.arrayBuffer, sourceBuffer.byteOffset + byteOffset, byteLength);
  const target = new Uint8Array(targetBuffer.arrayBuffer, targetBuffer.byteOffset + (bufferView.byteOffset || 0), bufferView.byteLength);
...
// dist/lib/extensions/KHR_meshopt_compression.js
...
// dist/lib/extensions/EXT_texture_avif.js
...
async function preprocess2(gltfData, options) {
  const scenegraph = new GLTFScenegraph(gltfData);
  const supportedImageFormats = await (0, import_images4.getSupportedImageFormats)();
  ...
  const { json } = scenegraph;
  for (const texture of json.textures || []) {
    const extension = scenegraph.getObjectExtension(texture, EXT_TEXTURE_AVIF);
    if (extension) {
      texture.source = extension.source;
    }
...
function transformTexCoords(materialIndex, gltfData, gltfScenegraph) {
  ...
  for (const textureInfo of materialTextures) {
    const extension = (_b = textureInfo.extensions) == null ? void 0 : _b[KHR_TEXTURE_TRANSFORM];
    if (extension) {
...
function findTextureInfos(value) {
  ...
  const textureInfos = [];
  const extensions = object.extensions;
  if (Number.isFinite(object.index) && (extensions == null ? void 0 : extensions[KHR_TEXTURE_TRANSFORM])) {
...
function transformPrimitives(gltfData, materialIndex, transformParameters) {
  ...
  if (primitives.length === 0 || primitives.some((primitive) => !canTransformPrimitive(gltfData, primitive, transformParameters.sourceTexCoord))) {
    return false;
  }
...
async function preprocessExtensions(gltf, options = {}, context) {
  ...
  for (const extension of extensions) {
    await ((_a = extension.preprocess) == null ? void 0 : _a.call(extension, gltf, options, context));
  }
}
...
function getGLTFImageOptions(options, mimeType) {
  ...
  const basisFormat = basisOptions == null ? void 0 : basisOptions.format;
  return {
    ...options,
    core: { ...options.core, mimeType },
    basis: {
      ...basisOptions,
      format: basisFormat && basisFormat !== "auto" ? basisFormat : (0, import_textures.selectSupportedBasisFormat)(basisOptions == null ? void 0 : basisOptions.supportedTextureFormats)
    }
  };
}
...
async function loadImage(gltf, image, index, options, context) {
  ...
  let parsedImage = await (0, import_loader_utils7.parseFromContext)(arrayBuffer, [import_images5.ImageLoader, import_textures.BasisLoader], gltfOptions, context);
...
var GLTFPostProcessor = class {
  ...
  _applySparseAccessor(accessor, sparse) {
    ...
    const sparseIndices = this._getTypedArrayFromBufferView(SparseIndexArray, this.getBufferView(sparse.indices.bufferView), sparse.indices.byteOffset || 0, sparse.count);
    const SparseValueArray = accessor.value.constructor;
    const sparseValues = this._getTypedArrayFromBufferView(SparseValueArray, this.getBufferView(sparse.values.bufferView), sparse.values.byteOffset || 0, sparse.count * accessor.components);
    for (let sparseValueIndex = 0; sparseValueIndex < sparse.count; sparseValueIndex++) {
      ...
      assert(Number.isInteger(accessorIndex) && accessorIndex >= 0 && accessorIndex < accessor.count, "glTF sparse accessor index is out of bounds");
  ...
  _getTypedArrayFromBufferView(ArrayType, bufferView, byteOffset, length) {
    const byteLength = length * ArrayType.BYTES_PER_ELEMENT;
    assert(byteOffset + byteLength <= bufferView.byteLength, "glTF sparse accessor data exceeds its buffer view");
    const buffer = bufferView.buffer;
    const absoluteByteOffset = buffer.byteOffset + (bufferView.byteOffset || 0) + byteOffset;
    const arrayBuffer = (0, import_loader_utils9.copyToArrayBuffer)(buffer.arrayBuffer, absoluteByteOffset, byteLength);
...
function normalizePrimitiveTopology(primitive, meshId, primitiveIndex) {
  ...
  const sourceIndices = (_a = primitive.indices) == null ? void 0 : _a.value;
  const sourceIndexCount = ((_b = primitive.indices) == null ? void 0 : _b.count) ?? getPrimitiveVertexCount(primitive);
...
function createLineLoopIndices(sourceIndices, sourceIndexCount, IndexArray) {
  ...
  for (let sourceIndex = 0; sourceIndex < sourceIndexCount; sourceIndex++) {
    ...
    generatedIndices[sourceIndex * 2 + 1] = getSourceIndex(sourceIndices, (sourceIndex + 1) % sourceIndexCount);
  }
